...
const firestoreFields = (value: Record<string, string>) => Object.fromEntries(Object.entries(value).map(([key, val]) => [key, { stringValue: val }]));
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const trackingId = `MS-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}-${crypto.randomUUID().slice(0, 6).toUpperCase()}`;
    ...
    const document = {
      fields: firestoreFields({ trackingId, name, email, mobile, service, package: requestPackage, budget, description, website, status: "Pending", timestamp }),
    };
    ...
    const safe = { trackingId: escapeHtml(trackingId), name: escapeHtml(name), email: escapeHtml(email), mobile: escapeHtml(mobile), service: escapeHtml(service), requestPackage: escapeHtml(requestPackage || "Not specified"), budget: escapeHtml(budget || "Not specified"), description: escapeHtml(description).replace(/\n/g, "<br>"), website: escapeHtml(website || "Not specified"), timestamp: escapeHtml(timestamp) };
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from,
        to: [site.email, email],
        subject: `MANAV SOLS Request Receipt — ${trackingId}`,
        html,
        reply_to: email,
      }),
    });
